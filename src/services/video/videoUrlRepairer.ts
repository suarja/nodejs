...
export class VideoUrlRepairer {
  ...
  async repairScenePlanWithAI(
    ...
    for (let i = 0; i <= maxRetries; i++) {
      try {
        logger.info(
          `🤖 Calling AI Judge for scene plan repair. Attempt ${i + 1}`
        );
        ...
        logger.info(
          `✅ AI Judge successfully repaired and validated the scene plan.`
        );
        return repairedPlan;
      } catch (error) {
        lastError = error;
        logger.warn(
          `⚠️ AI Judge repair attempt ${i + 1} failed: ${
            error instanceof Error ? error.message : "Unknown error"
          }`
        );
        if (i === maxRetries) {
          logger.error(
            `❌ AI Judge failed to repair the scene plan after ${
              maxRetries + 1
            } attempts.`
          );
  ...
  repairUrl(
    ...
    if (incorrectUrl !== correctUrl) {
      console.warn(`⚠️ Impossible de réparer l'URL (${step}):`, {
        incorrectUrl,
  ...
  validateTemplate(template: any): void {
    ...
    if (invalidUrls.length > 0) {
      throw new Error(
        `URLs invalides détectées dans le template: ${invalidUrls.join(", ")}`
      );
    }

    console.log("✅ Toutes les URLs du template sont valides");
  }
